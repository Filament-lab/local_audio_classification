...
class FrequencyDomainSubClassExperiment(BaseExperiment):
    ...
    def process_dataset(self):
        ...
        self.classes = self.label_df[self.config.sub_class_column_name].unique()
        self.num_classes = len(self.classes)
        logger.info(f"{self.num_classes} classes found")
        logger.info(f"{self.label_df[self.config.sub_class_column_name].unique()}")
        self.encoder = LabelEncoder()
        self.encoder.fit(self.label_df[self.config.sub_class_column_name].unique())
        self.label_array = self.encoder.transform(self.label_df[self.config.sub_class_column_name].to_numpy())
...
if __name__ == "__main__":
    ...
    Experiment = FrequencyDomainSubClassExperiment(CFG)
